...
class SeleniumExtractor:
    ...
    def get_louisiana(self):
        ...
        button1 = browser.find_elements_by_css_selector(
            "div.flex-fluid.overflow-hidden"
        )
        time.sleep(2)
        day = self.today
    ...
    def get_oklahoma(self):
        ...
        data = [
            e.text for e in data if e.text and e.text[0] >= "0" and e.text[0] <= "9"
        ]
        ...
        if not os.access("data/{}/oklahoma.json".format(day), os.F_OK):
            ...
            ##
            doc = fitz.Document("pdfs/oklahoma/{}.pdf".format(day))
            # find the page
            lines = doc.getPageText(0).splitlines()
            total = int(lines[2].replace(",", ""))
            age_data = {}
            for i in data:
                age_data[i.split()[0]] = int(
                    round(total * float(i.split()[1][0:-1]) * 0.01, 0)
                )
            path = "data/{}".format(day)
            if not os.path.exists(path):
                os.mkdir(path)
    ...
    def get_oklahoma2(self):
        ...
        for i in range(date_diff.days + 1):
            ...
            url = "https://coronavirus.health.ok.gov/sites/g/files/gmc786/f/eo_-_covid-19_report_-_{}.pdf".format(
                day
            )
            if url.split("/")[-1] not in existing_assets:
                if requests.get(url).status_code == 200:
                    with open("pdfs/oklahoma/" + url.split("/")[-1], "wb") as f:
                    ...
                    if not os.path.exists(path):
                        os.mkdir(path)
                    with open(
                        "data/{}/oklahoma2.json".format(dayy.strftime("%Y-%m-%d")), "w"
                    ) as f:
                        json.dump(age_data, f)
                    print("\n------ Processed Oklahoma2 {} ------\n".format(day))
                    doc.close()
                    ...
                    print(
                        "Warning: Report for Oklahoma {} is not available".format(day)
                    )
        ...
        if not os.access("data/{}/NorthDakota.json".format(day), os.F_OK):
            ...
            for i in range(9):
                data = browser.find_element_by_xpath(
                    '//*[@id="pvExplorationHost"]/div/div/exploration/div/explore-canvas-modern/div/div[2]/div/div[2]/div[2]/visual-container-repeat/visual-container-modern[20]/transform/div/div[3]/div/visual-modern/div/*[name()="svg"]/*[name()="svg"]/*[name()="g"][1]/*[name()="g"][2]/*[name()="svg"]/*[name()="g"][2]/*[name()="rect"]['
                    + str(i + 1)
                    + "]"
                )
                data = data.get_attribute("aria-label")
                age_data[data.split(".")[0].split()[-1]] = data.split(".")[1].split()[
                    -1
                ]
            if age_data:
                path = "data/{}".format(day)
    ...
    def get_az(self):
        ...
        try:
            ...
            print(err)
            print(
                "==> Report for Arizona {} is not available".date.today().strftime(
                    "%Y-%m-%d"
                )
            )
        else:
    ...
    def get_nc2(self):
        ...
        try:
            ...
            print(err)
            print("==> Report for North Carolina {} is not available".format(day))
        else:
    ...
    def get_mississippi(self):
        ...
        for i in range(date_diff.days + 1):
            ...
            url = (
                "https://msdh.ms.gov/msdhsite/_static/images/graphics/covid19-chart-age-"
                + str(day[5:])
                + ".png"
            )
            if day + ".png" not in existing_assets:
                try:
                    ...
                    print(err)
                    print("==> Report for Mississippi {} is not available".format(day))
                else:
    ...
    def get_missouri(self):
        ...
        if not os.access("data/{}/missouri.json".format(day), os.F_OK):
            ...
            data = [
                ...
                for e in data
                if e.get_attribute("aria-label")
                and "Count" in e.get_attribute("aria-label")
            ]
            time.sleep(2)
            age_data = {}
    ...
    def get_kentucky(self):
        ...
        data = [
            e.get_attribute("aria-label") for e in data if e.get_attribute("aria-label")
        ]
        browser.implicitly_wait(5)
    ...
    def get_delware(self):
        ...
        if not os.access("data/{}/delaware.json".format(day), os.F_OK):
            ...
            browser.find_element_by_xpath(
                "/html/body/main/div/div/div[2]/section/form/button"
            ).click()
            ...
            if browser.execute_script("return document.readyState") == "complete":
                ...
                for i in range(6):
                    path1 = browser.find_element_by_xpath(
                        '//*[@id="total-deaths-by-age"]/div[2]/div/table/tbody/tr['
                        + str(i + 1)
                        + "]/td[1]/span/span"
                    )
    ...
    def get_vermont(self):
        ...
        if not os.access("data/{}/vermont.json".format(day), os.F_OK):
            ...
            for i in range(6):
                data = browser.find_element_by_xpath(
                    '//*[@id="ember252"]/div/div/*[name()="svg"]/*[name()="g"][7]/*[name()="g"]/*[name()="g"]/*[name()="g"]['
                    + str(i + 1)
                    + "]"
    ...
    def get_indiana(self):
        ...
        try:
            ...
            print(
                err,
                "\n ==> Report for  Indiana {} is not available".format(
    ...
    def get_oregon(self):
        ...
        try:
            ...
            print(
                err,
                "\n ==> Report for Oregon {} is not available".format(
                    self.today.strftime("%Y-%m-%d")
                ),
            )
        else:
    ...
    def get_nevada(self):
        ...
        browser.find_element_by_xpath(
            "/html/body/div[1]/ui-view/div/div[2]/logo-bar/div/div/div/logo-bar-navigation/span/a[2]/span/span[2]"
        ).click()
        time.sleep(2)
        browser.implicitly_wait(2)
        browser.find_element_by_xpath(
            '//*[@id="flyoutElement"]/div[1]/div/div/ul/li[1]/a'
        ).click()
        day = browser.find_element_by_xpath(
            '//*[@id="pvExplorationHost"]/div/div/exploration/div/explore-canvas-modern/div/div[2]/div/div[2]/div[2]/visual-container-repeat/visual-container-modern[10]/transform/div/div[3]/div/visual-modern/div/div/div/p[2]/span'
        ).text
        day = day.split()[-1]
        day = parsedate(day).strftime("%Y-%m-%d")
        time.sleep(10)
        if not os.access("data/{}/nevada.json".format(day), os.F_OK):
            ...
            browser.find_element_by_xpath(
                '//*[@id="flyoutElement"]/div[1]/div/div/ul/li[7]/a'
            ).click()
            ...
            browser.find_element_by_xpath(
                '//*[@id="pvExplorationHost"]/div/div/exploration/div/explore-canvas-modern/div/div[2]/div/div[2]/div[2]/visual-container-repeat/visual-container-modern[1]/transform/div/div[3]/div/visual-modern/div/div/div[2]/div'
            ).click()
            ...
            browser.implicitly_wait(6)
            browser.find_element_by_xpath(
                "/html/body/div[6]/div[1]/div/div[2]/div/div[1]/div/div/div[2]/div/span"
            ).click()
            ...
            for i in range(8):
                data = browser.find_element_by_xpath(
                    '//*[@id="pvExplorationHost"]/div/div/exploration/div/explore-canvas-modern/div/div[2]/div/div[2]/div[2]/visual-container-repeat/visual-container-modern[2]/transform/div/div[3]/div/visual-modern/div/*[name()="svg"]/*[name()="svg"]/*[name()="g"][1]/*[name()="g"][3]/*[name()="svg"]/*[name()="g"]['
                    + str(i + 1)
                    + ']/*[name()="rect"]'
                ).get_attribute("aria-label")
    ...
    def get_michigan(self):
        ...
        time.sleep(200)
        day = browser.find_element_by_xpath(
            '//*[@id="pvExplorationHost"]/div/div/exploration/div/explore-canvas-modern/div/div[2]/div/div[2]/div[2]/visual-container-repeat/visual-container-group[2]/transform/div/div[2]/visual-container-modern[2]/transform/div/div[3]/div/visual-modern/div/*[name()="svg"]/*[name()="g"][1]/*[name()="text"]/*[name()="tspan"]'
        ).text
        time.sleep(10)
        day = parsedate(day).strftime("%Y-%m-%d")
        ...
        if not os.access("data/{}/michigan.json".format(day), os.F_OK):
            ...
            browser.implicitly_wait(2)
            total = browser.find_element_by_xpath(
                '//*[@id="pvExplorationHost"]/div/div/exploration/div/explore-canvas-modern/div/div[2]/div/div[2]/div[2]/visual-container-repeat/visual-container-modern[2]/transform/div/div[3]/div/visual-modern/div/div/div[2]/div[1]/div[4]/div/div/div[3]/div[1]'
            ).text
            browser.implicitly_wait(5)
            browser.find_element_by_xpath(
                '//*[@id="pvExplorationHost"]/div/div/exploration/div/explore-canvas-modern/div/div[2]/div/div[2]/div[2]/visual-container-repeat/visual-container-group[3]/transform/div/div[2]/visual-container-modern[4]/transform/div/div[3]/div/visual-modern/div/button'
            ).click()
            browser.implicitly_wait(25)
            time.sleep(30)
            browser.find_element_by_xpath(
                '//*[@id="pvExplorationHost"]/div/div/exploration/div/explore-canvas-modern/div/div[2]/div/div[2]/div[2]/visual-container-repeat/visual-container-group/transform/div/div[2]/visual-container-modern[2]/transform/div/div[3]/div/visual-modern/div/button'
            ).click()
            browser.implicitly_wait(25)
            time.sleep(20)
            browser.implicitly_wait(5)
            ...
            age_data = [
                ...
                for e in data
                if e.get_attribute("aria-label")
                and "Total Deaths" in e.get_attribute("aria-label")
                and "Age Group" in e.get_attribute("aria-label")
            ]
            data = age_data
            age_data = {}
            age_data["0-19"] = "0"
    ...
    def get_washington(self):
        ...
        time.sleep(20)
        day = browser.find_element_by_xpath(
            '//*[@id="dnn_ctr34535_HtmlModule_lblContent"]/p[3]/strong'
        ).text
        time.sleep(10)
        day = day.split()[-1]
        day = parsedate(day).strftime("%Y-%m-%d")
        if not os.access("data/{}/washington.json".format(day), os.F_OK):
            ...
            time.sleep(2)
            total = browser.find_element_by_xpath(
                '//*[@id="pnlConfirmedCasesDeathsTbl"]/div/div/table/tbody/tr[41]/td[3]'
            ).text
            time.sleep(2)
            age_data = {}
            for i in range(6):
                group = browser.find_element_by_xpath(
                    '//*[@id="pnlCasesDeathsByAgeTbl"]/div/div/table/tbody/tr['
                    + str(i + 1)
                    + "]/td[1]"
                ).text
                data = browser.find_element_by_xpath(
    ...
    def get_illinois(self):
        ...
        time.sleep(2)
        day = parsedate(
            browser.find_element_by_xpath('//*[@id="updatedDate"]').text
        ).strftime("%Y-%m-%d")
            ...
            for j in range(9):
                ...
                for i in range(8):
                    s = browser.find_elements_by_xpath(
                        '//*[@id="pieAge"]/div/div/*[name()="svg"][1]/*[name()="g"][4]/*[name()="g"]/*[name()="g"][8]/*[name()="g"][1]/*[name()="g"]['
                        + str(i + 1)
                        + ']/*[name()="g"]/*[name()="g"]['
                        + str(j + 1)
                        + ']/*[name()="text"]'
    ...
    def get_utah(self):
        ...
        if not os.access("data/{}/utah.json".format(day), os.F_OK):
            ...
            browser.implicitly_wait(2)
            total = browser.find_element_by_xpath(
                '//*[@id="DataTables_Table_2"]/tbody/tr/td[1]'
            ).text
            age_data = {}
            browser.implicitly_wait(1)
            for i in range(8):
                group = browser.find_element_by_xpath(
                    '//*[@id="DataTables_Table_3"]/tbody/tr[' + str(i + 1) + "]/td[1]"
                ).text
                data = browser.find_element_by_xpath(
                    '//*[@id="DataTables_Table_3"]/tbody/tr[' + str(i + 1) + "]/td[3]"
                ).text
                age_data[group] = data
            age_data["total"] = total
            path = "data/{}".format(day)
    ...
    def get_wyoming(self):
        ...
        try:
            ...
            print(err)
            print(
                "==> Report for Wyoming {} is not available".date.today().strftime(
                    "%Y-%m-%d"
                )
            )
        else:
    ...
    def get_california(self):
        ...
        try:
            ...
            print(err)
            print(
                "==> Report for California {} is not available".date.today().strftime(
                    "%Y-%m-%d"
                )
            )
        else:
    ...
    def get_sc(self):
        ...
        try:
            ...
            print(err)
            print(
                "==> Report for SouthCarolina {} is not available".date.today().strftime(
                    "%Y-%m-%d"
                )
            )
        else:
            ...
            if not os.access("data/{}/SouthCarolina.json".format(date), os.F_OK):
                ...
                doc.close()
                shutil.move(
                    f"pdfs/SouthCarolina/{date}.pdf", f"pdfs/SouthCarolina/{day}.pdf"
                )
            else:
    ...
    def get_nh(self):
        ...
        try:
            ...
            print(err)
            print(
                "==> Report for NewHampshire {} is not available".date.today().strftime(
                    "%Y-%m-%d"
                )
            )
        else:
            ...
            if not os.access("data/{}/new_hampshire.json".format(date), os.F_OK):
                ...
                doc.close()
                shutil.move(
                    f"pdfs/new_hampshire/{date}.pdf", f"pdfs/new_hampshire/{day}.pdf"
                )
    ...
    def get_kansas(self):
        ...
        time.sleep(10)
        day = browser.find_element_by_xpath(
            '//*[@id="ng-app"]/body/div[1]/div[2]/section/div/div[2]/section[3]/div/figcaption/div[2]/div/div[2]/dl/div[3]/dd/span'
        ).text
        day = parsedate(day).strftime("%Y-%m-%d")
        browser.close()
        browser.quit()
        ...
        try:
            ...
            print(err)
            print("==> Report for kansas {} is not available".day)
        else:
    ...
    def get_hawaii(self):
        ...
        try:
            ...
            print(err)
            print(
                "==> Report for hawaii {} is not available".date.today().strftime(
                    "%Y-%m-%d"
                )
            )
        else:
    ...
    def get_colorado_pngs(self):
        ...
        browser.implicitly_wait(30)
        browser.find_element_by_xpath(
            '//*[@id="view12902688137728866763_889995828657056411"]'
        ).click()
        time.sleep(50)
        browser.find_element_by_xpath(
            '//*[@id="view12902688137728866763_9910513099237291342"]/div[1]/div[2]/canvas[2]'
        ).click()
        browser.implicitly_wait(50)
        width = browser.execute_script("return document.documentElement.scrollWidth")
        height = browser.execute_script("return document.documentElement.scrollHeight")
    ...
    def get_ri(self):
        ...
        try:
            ...
            print(
                err,
                "\n ==> Report for Rhode Island {} is not available".format(
                    self.today.strftime("%Y-%m-%d")
                ),
            )
        else:
    ...
    def get_idaho(self):
        ...
        try:
            ...
            print(err)
            print(
                "==> Report for idaho {} is not available".date.today().strftime(
                    "%Y-%m-%d"
                )
            )
        else:
